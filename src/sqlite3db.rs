...
impl db::Database for Sqlite3Database {
    ...
    fn get_board(&self, board_id: u64) -> Result<site::Board, PlainchantErr> {
        ...
        query.query_row((board_id,), row_to_board).map_err(|e| e.into())
    }
    ...
    fn get_original(&self,
        ...
        query.query_row((board_id, post_num), row_to_original).map_err(|e| e.into())
    }
    ...
    fn get_post(&self,
                board_id: u64,
                post_num: u64)
                -> Result<Box<dyn site::Post>, PlainchantErr> {
        let conn = self.pool.get()?;
        let mut query = conn.prepare(r#"
            SELECT BoardId, PostNum, Time, Ip, Poster, Body,
                   FeatherType, FeatherText, FileId, FileName, OrigNum FROM Posts 
